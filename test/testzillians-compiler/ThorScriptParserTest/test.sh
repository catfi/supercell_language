...
for ARG in "$@"; do
    ...
    if [ $MODE -eq 1 ]; then
        $EXEC $ARG --root-dir=`dirname $ARG` --mode-parse --debug-parser-ast-with-loc --enable-static-test
        ERROR_CODE="$?"
    ...
    if [ $MODE -eq 2 ]; then
        $EXEC $ARG --root-dir=`dirname $ARG` --mode-xform --debug-restructure-stage --enable-static-test
        ERROR_CODE="$?"
        if [ $ERROR_CODE -ne 0 ]; then
    ...
    $EXEC $ARG --root-dir=`dirname $ARG` --mode-parse --debug-parser --enable-static-test |& grep -v "[DEBUG]" > $TEMP_FILE_A
